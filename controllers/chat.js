...
const removeMembers = TryCatch(async(req, res, next)=>{
    ...
    if(chat.creator.toString() !== req.user.toString()) return next(new ErrorHandler('You are not authorized to remove members', 403));
    ...
    emitEvent(req, ALERT, chat.members, `${userWhoWillBeRemoved.name} has been removed from the group`);
...
const leaveGroup = TryCatch(async(req, res, next)=>{
    ...
    const remainingMem = chat.members.filter((member) =>member.toString() !== req.user.toString());
